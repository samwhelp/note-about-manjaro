...
is_debug () {
	if [ "$DEBUG_NVIMRC_CTRL" = "true" ]; then
		return 0
	fi
...
base_var_init () {
	...
	if [ "none${THE_CMD_FILE_NAME}" = 'none' ]; then
		THE_CMD_FILE_NAME="nvimrc-ctrl"
	fi

	THE_CMD_FILE_PATH="${THE_CMD_DIR_PATH}/$THE_CMD_FILE_NAME"

	## https://github.com/samwhelp/play-ubuntu-20.04-plan/blob/master/project/vimrc-profile/nvimrc-ctrl/nvimrc-ctrl
	THE_SELF_UPDATE_URL='https://raw.githubusercontent.com/samwhelp/note-about-manjaro/gh-pages/_demo/project/vimrc-profile/nvimrc-ctrl/nvimrc-ctrl'
	...
	## Nvim
	THE_NVIMRC_DIR_NAME="nvim"
	THE_NVIMRC_DIR_PATH="$HOME/.config/$THE_NVIMRC_DIR_NAME"
...
base_var_dump () {
	...
	util_debug_echo "THE_NVIMRC_DIR_NAME=$THE_NVIMRC_DIR_NAME"
	util_debug_echo "THE_NVIMRC_DIR_PATH=$THE_NVIMRC_DIR_PATH"
